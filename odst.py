...
'''
The Oblivious Differentiable Trees adopted form https://github.com/Qwicen/node
'''

class ODST(ModuleWithInit):
    def __init__(self, in_features, num_trees, depth=6, tree_dim=1, flatten=True,
        ...
        :param flatten: if False, returns [..., num_trees, tree_dim],
        ...
        """
        super().__init__()
        self.depth, self.num_trees, self.tree_dim, self.flatten = depth, num_trees, tree_dim, flatten
    ...
    def forward(self, input):
        ...
        return response.flatten(1, 2) if self.flatten else response

    def initialize(self, input, eps=1e-6):
        # data-aware initializer
        assert len(input.shape) == 2
        #if input.shape[0] < 1000:
            #print("Data-aware initialization is performed on less than 1000 data points. This may cause instability."
        ...
        with torch.no_grad():
            ...
            self.feature_thresholds.data[...] = torch.as_tensor(
                list(map(np.percentile, self.to_numpy(feature_values.flatten(1, 2).t()), percentiles_q.flatten())),
            ...
            temperatures = np.percentile(self.to_numpy(abs(feature_values - self.feature_thresholds)),
    ...
    def __repr__(self):
        return "{}(in_features={}, num_trees={}, depth={}, tree_dim={}, flatten={})".format(
            self.__class__.__name__, self.feature_selection_logits.shape[0],
            self.num_trees, self.depth, self.tree_dim, self.flatten
        )
