...
def to_one_hot(y, num=None):
    return nn.functional.one_hot(y, num_classes= num)

def _make_ix_like(X, dim):
    idx_ran = torch.arange(1, X.size(dim) + 1, device=X.device, dtype=X.dtype)
    view = [1] * X.dim()
    view[0] = -1
    return idx_ran.view(view).transpose(0, dim)
